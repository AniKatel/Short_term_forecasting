...
# кросс-валидация
def CrossValidation(params, method, ts, p1, p2):
    tscv = TimeSeriesSplit(n_splits=5)
    err = []
    for train, test in tscv.split(ts):
        # предсказанные значения
        #predict = method(params, train, p1, p2, 48)[24:]
        predict = method(params, ts[train], p1, p2, len(test))
        # настоящие значения
        #actual = test
        actual = ts[test]
...
def double_seasonal_additiveHoltWinters(params, ts, p1, p2, h):
    ...
    # построение прогноза
    for i in range(0, h):
        predict.append(level + (i + 1) * trend + season1[i % p1] + season2[i % p2])
    return predict
